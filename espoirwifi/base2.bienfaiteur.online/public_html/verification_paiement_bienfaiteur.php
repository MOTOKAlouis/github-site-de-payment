...
<?php
  ...
  //verification si l'identifiant n'a pas encore été utiliser
    ...
    $query = "SELECT * FROM ticket_bienfaiteur WHERE identifier=:identifiant ";
...
Voici Votre Ticket Wifi :
                               ...
                               echo "<br>";
                          echo "
                           <input type='hidden' class='code_wifi' value= ".$_SESSION["ticket"].">
                           <input type='hidden' class='pass_wifi' value= ".$_SESSION["pass_ticket"]." >
                           <input type='hidden' class='montant_transac' value= ".$_SESSION["montant_transac"]." >
                              "; ?>
                              ...
                              var montant_transac = $('.montant_transac').val()
                                ...
                                html: "<table id='captureTable' class='table table-striped captureTicket'><tr><th>USERNAME</th><th>PASSWORD</th><th>COUT</th></tr><tr><td>"+code_wifi+"</td><td>"+pass_wifi+"</td><td>"+montant_transac+"</td></tr></table><input id ='capture' type='submit' name='codeTicket' value='ENREGISTREZ LE CODE' >",
                                focusConfirm: false,
...
//verification du paiement par le code de reference
          ...
          $curl = curl_init();
          $data = array ( "auth_token" => "5549e4f1-5d96-4985-b4e1-032790e5f5f4", "identifier" => $identifiant);
            ...
            if ($etat==0){
              ...
              if (!isset($conn)){
                require 'connexion.php';
                  ...
                  $sql = 'SELECT * 
                  FROM ticket_bienfaiteur WHERE statut = 0 AND montant ='.$montant_transaction.' limit 0,1';
       ...
       $sql = "UPDATE ticket_bienfaiteur SET statut=:statut, date_achat_code=:date_achat, heure_achat_code=:heure_achat, identifier=:identifier WHERE id=:id";
                        $stmt= $conn->prepare($sql);
                        $stmt->execute($data);
                           //on verifie si tout va bien
                           if ($stmt){
									 if(!isset($_POST['marker'])){
                          include_once 'phpmail_bienfaiteur.php';
                           }
...
<?php
                           echo "<br>
  <a class='btn btn-primary' data-toggle='collapse' href='http://b.com/login.html' role='button' aria-expanded='false' aria-controls='collapseExample'>
...
var lienRetour = " <a class='btn btn-primary' data-toggle='collapse' href='http://b.com/login.html' role='button' aria-expanded='false' aria-controls='collapseExample'>RETOUR A LA PAGE DE CONNEXION</a>";
                          ...
                          Avez vous validé la transaction sur votre mobile? si Non veuillez Ressayer...
                                ...
                                ?>
                                  <script>
                                      ...
                                      icon: 'error',
                                      title: 'Oops...',
                                      text: 'Avez Vous valider la Transaction ?',
                                      footer: '<a href="">Sinon recommencer</a>'
                                    })
                                  </script>
